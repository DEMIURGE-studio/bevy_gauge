...
use crate::prelude::{Expression, Stats, StatRequirement, StatType};
        ...
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for StatTypeVisitor {
            ...
            where
                E: de::Error,
            {
                Ok(StatType::from_float(value as f32))
            }
            ...
            where
                E: de::Error,
            {
                Ok(StatType::from_expression(value.into()))
            }
...
fn flatten_nested_stats(
    ...
    out: &mut HashMap<String, StatType>,
) {
